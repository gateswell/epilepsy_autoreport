...
$mw->geometry("835x345");
$mw->title('抗癫痫药物过敏HLA基因筛查与咨询报告生成系统V1.5.180420');
...
$year = $year + 1900;
my $now = $year.'/'.$thismon.'/'.$thisday.' '.$hour.':'.$min;
...
my $detectorEntry = $frame4->Entry(-textvariable=> '王亚男 喻聪',-width=>50)->pack(-side => 'left');
...
my $checkerInfoEntry = $frame5->Entry(-text => '谢志聪',-width=>50)->pack(-side=> 'left');
...
my $label_frame = $mw -> LabFrame(-label => $now,-labelside => "acrosstop")->pack(-side => 'left');
$label_frame -> Label(-text => "Author\:Cao Shuhuan\n") ->pack(-side => 'left');
...
my($Sampledir,$missiondir,$Typingdir,$Destdir,$checkername,$detectorname,$signername,$rechedate,$signdate,$sample_id_header); 
sub get_all{
...
sub report_get{	
	...
	my %origin_name;	#报告单编号为键，原始样本编号为值
	my ($province,$sample_id,$origin_sample_id,$receive_date,$collect_date,$report_id,$name,$gender,$age,$send_hospital,$send_department,$send_doctor);
	for my $worksheet($workbook_1->worksheet(0)){
		...
		for my $row(($row_min+1) .. $row_max){
			...
			$origin_sample_id= $cell? encode ('GB2312', $cell->value()) :'';
			if($origin_sample_id =~ /^(\w)(\d+\w+)/){
				$sample_id = $2;
				$sample_id_header = $1;
				$origin_name{$sample_id} = $origin_sample_id;
			}
			elsif($origin_sample_id =~ /^(\d+\w+)/){
				$sample_id = $1;
				$origin_name{$sample_id} = $origin_sample_id;
			}
			...
			my $cell = $worksheet ->get_cell($row,$col_min+24);
			...
			if($report_id){
				...
				$collect_hash{$report_id} = $collect_date;
				#$receive_hash{$report_id} = $receive_date_1;
				$receive_hash{$report_id} = $receive_date;
			...
			else{$mw->messageBox(-message=>"生产信息中缺少报告单编号\:$report_id".'，请修改！',-type=>'ok');}
	...
	for my $worksheet ($workbook -> worksheets()){
		...
		for my $row($row_min..$row_max){	#从第1行开始读取，直到没有了样本名为止的行
			...
			$report_alias = $tmp[0];
			if($report_alias=~ /^\w(\d+\w+)/){
				$report_alias=$1;
			}
			elsif($report_alias=~ /^(\d+\w+)/){
				$report_alias=$1;
			}
			$alias{$sample_type} = $report_alias;	#将全称和简称分别作为%alias的键值
			$result_report = 'CSTB'.$report_alias;	#报告单编号
			if ($report_id_array_var !~ /$result_report/){
				$mw->messageBox(-message=>"导出文件中报告单编号\:$result_report"."缺少或多余，请回到上一步检查是否选择错误的导出文件，",-type=>'ok');
				exit;
			}
	...
	my $format8_7 = $workbook -> add_format();
	$format8_7 ->set_color('black');
	$format8_7 ->set_pattern();
	#$format8_7 ->set_bold();
	$format8_7 ->set_bg_color('yellow');
	$format8_7 ->set_size(12);
	$format8_7 ->set_font('Times New Roman');
	$format8_7 ->set_align('center');
	$format8_7 ->set_align('vcenter');
	
	
	my $format8_8 = $workbook -> add_format();
	$format8_8 ->set_color('black');
	$format8_8 ->set_size(12);
	$format8_8 ->set_font('Times New Roman');
	$format8_8 ->set_align('center');
	$format8_8 ->set_align('vcenter');
	
	my $format8_9 = $workbook -> add_format();
	$format8_9 ->set_color('black');
	$format8_9 ->set_pattern();
	#$format8_9 ->set_bold();
	$format8_9 ->set_bg_color('yellow');
	$format8_9 ->set_size(12);
	$format8_9 ->set_font('Times New Roman');
	$format8_9 ->set_align('center');
	$format8_9 ->set_align('vcenter');
	
	
	my $format8_10 = $workbook -> add_format();
	$format8_10 ->set_color('black');
	$format8_10 ->set_size(12);
	$format8_10 ->set_font('Times New Roman');
	$format8_10 ->set_align('center');
	$format8_10 ->set_align('vcenter');
	...
	my $format26 = $workbook -> add_format();
	$format26 ->set_size(12);
	$format26 ->set_font('宋体');
	$format26 ->set_align('center');
	$format26 ->set_align('vcenter');
	
	my $format27 = $workbook -> add_format();
	$format27 ->set_size(12);
	$format27 ->set_font('Times New Roman');
	$format27 ->set_align('left');
	$format27 ->set_align('vcenter');
	
	foreach my $name(@unique_samples){	#按样本数产生sheet数
		my $sheetname_full = 'CSTB'.$name;	#报告单编号
		my $sheetname      = $sample_id_header.$name;	#样本编号,如D2017EM00024 
		...
		$worksheet ->merge_range('A2:J2','深圳荻硕贝肯医学检验实验室',$format1);
		...
		my $sample_id_result = $sample_id_header.$report_sheet{$sheetname_full};
		$worksheet ->merge_range('E11:J11',"样本编号：$sample_id_result",$format15);
	...
	my $worksheet_sum = $workbook -> add_worksheet('结果汇总');
	$worksheet_sum -> freeze_panes(1,0);
	$worksheet_sum->set_column('A:A',9.88);
	$worksheet_sum->set_column('B:B',9.88);
	$worksheet_sum->set_column('C:C',13.63);
	$worksheet_sum->set_column('K:K',40.63);
	$worksheet_sum -> write('A1','检测日期',$format26);
	$worksheet_sum -> write('B1','报告日期',$format26);
	$worksheet_sum -> write('C1','样本编号',$format26);
	$worksheet_sum -> write('D1','姓名',$format26);
	$worksheet_sum -> write('E1','癫痫基因',$format26);
	$worksheet_sum -> write('F1','级别',$format26);
	my $sample_count =0;
	for my $sample_id_sum(@unique_samples){
		my $sample_full_id = 'CSTB'.$sample_id_sum;	#报告单编号
		$worksheet_sum -> write($sample_count+1, 0, $mission{$sample_full_id},$format27);
		$worksheet_sum -> write($sample_count+1, 1, $rechedate,$format27);
		#$worksheet_sum -> write($sample_count+1, 2, $sample_full_id);
		$worksheet_sum -> write($sample_count+1, 2, $origin_name{$sample_id_sum},$format27);
		$worksheet_sum -> write($sample_count+1, 3, decode('GB2312',$report_patient{$sample_full_id}),$format26);
		$worksheet_sum -> write($sample_count+1, 4, $PosorNeg{$sample_id_sum},$format26);
		$worksheet_sum -> write($sample_count+1, 5, $jingshi{$sample_id_sum},$format26);
		
		my $HLA_A_1 = ${$typing_A_results{$sample_id_sum}}[0];
		my $HLA_A_2 = ${$typing_A_results{$sample_id_sum}}[1];
		if($A_positive_type =~ /$HLA_A_1/){
			$worksheet_sum ->write($sample_count+1, 6, 'A*'.$HLA_A_1,$format8_7);	#加粗加亮的模式
		}
		else{
			$worksheet_sum ->write($sample_count+1, 6 ,'A*'.$HLA_A_1,$format8_8);
		}
		if($A_positive_type =~ /$HLA_A_2/){
			$worksheet_sum ->write($sample_count+1, 7, 'A*'.$HLA_A_2,$format8_9);	#加粗加亮的模式
		}
		else{
			$worksheet_sum ->write($sample_count+1, 7, 'A*'.$HLA_A_2,$format8_10);
		}

		my $HLA_B_1 = ${$typing_B_results{$sample_id_sum}}[0];
		my $HLA_B_2 = ${$typing_B_results{$sample_id_sum}}[1];
		if($B_type =~ /$HLA_B_1/){
			$worksheet_sum ->write($sample_count+1, 8, 'B*'.$HLA_B_1,$format8_7);	#加粗加亮的模式
		}
		else{
			$worksheet_sum ->write($sample_count+1, 8, 'B*'.$HLA_B_1,$format8_8);
		}
		if($B_type =~ /$HLA_B_2/){
			$worksheet_sum ->write($sample_count+1, 9, 'B*'.$HLA_B_2,$format8_9);	#加粗加亮的模式
		}
		else{
			$worksheet_sum ->write($sample_count+1, 9, 'B*'.$HLA_B_2,$format8_10);
		}
		my $pdf_title = $origin_name{$sample_id_sum}.decode('GB2312',$hosptal_hash{$sample_full_id}).'-'.decode('GB2312',$report_patient{$sample_full_id});
		$worksheet_sum -> write($sample_count+1, 10, $pdf_title,$format27);
		$sample_count ++;
	}
